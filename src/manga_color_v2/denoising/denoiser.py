...
class FFDNetDenoiser:
    ...
    def get_denoised_image(self, imorig, sigma = None):
        ...
        if max(imorig.shape[0], imorig.shape[1]) > 1200:
            ratio = max(imorig.shape[0], imorig.shape[1]) / 1200.0
            new_w = int(imorig.shape[1] / ratio)
            new_h = int(imorig.shape[0] / ratio)
            # Ensure the new dimensions are at least 1 pixel.
            if new_w > 0 and new_h > 0:
                imorig = cv2.resize(imorig, (new_w, new_h), interpolation=cv2.INTER_AREA)
        ...
        # The original FFDNetDenoiser returns a BGR uint8 image, 
        # but the rest of our code expects an RGB float32 [0,1] image.
        # Convert it here for consistency.
        outim_bgr_uint8 = variable_to_cv2_image(outim)
        outim_rgb_uint8 = cv2.cvtColor(outim_bgr_uint8, cv2.COLOR_BGR2RGB)
        return outim_rgb_uint8.astype(np.float32) / 255.0
